...
function run() {
  try {
    ...
    const LaundryBlogsCollection = client
      .db("WorkingTitle")
      .collection("laundry-blogs");

    // extension for city laundry
    app.get("/laundry-blogs", async (req, res) => {
      try {
        const blogs = await LaundryBlogsCollection.find({}).toArray();
        res.json({
          success: true,
    ...
    app.get("/laundry-blogs/:slug", async (req, res) => {
      const blogSlug = req.params.slug;
      try {
        const blog = await LaundryBlogsCollection.findOne({
    ...
    app.post("/wishlist", async (req, res) => {
      ...
      if (exist) {
        return res.send({
          success: false,
          message: "Item Already added to the wishlist",
        });
      }
    ...
    app.post("/orders/placeorder", async (req, res) => {
      ...
      const cartItems = await CartsCollection.find({
